...
class DeviceServiceImpl implements DeviceServiceInt {
  ...
  /// Registered refreshed callback for cleanup during disconnect.
  Future<void> Function()? _registeredOnRefreshedCallback;
  ...
  Future<Either<RepositoryFailure, Unit>> updateFcmToken({
    ...
    logd('DeviceService: updateFcmToken called with token: ${fcmToken != null ? '***' : 'null'}');
    ...
    try {
      ...
      final data = Map<String, dynamic>.from(result.data as Map);
      if (data['success'] != true) {
        logw('DeviceService: updateFcmToken response indicated failure');
  ...
  Future<void> connectToAuthService({
    ...
    _registeredOnRefreshedCallback = onRefreshed ?? _defaultOnRefreshed;
    ...
    _authService!.addOnRefreshedCallback(_registeredOnRefreshedCallback!);
  ...
  void _disconnectFromAuthService() {
    ...
    if (_registeredOnRefreshedCallback != null) {
      _authService!.removeOnRefreshedCallback(_registeredOnRefreshedCallback!);
    }
    ...
    _registeredOnRefreshedCallback = null;
  ...
  /// Default callback for refreshed event.
  Future<void> _defaultOnRefreshed() async {
    logd('DeviceService: onRefreshed triggered');
    // Re-register device to keep metadata fresh
    final result = await registerDevice();
    result.fold(
      (failure) => logw('DeviceService: registerDevice failed on refresh: $failure'),
      (_) => logd('DeviceService: registerDevice succeeded on refresh'),
    );
  }
