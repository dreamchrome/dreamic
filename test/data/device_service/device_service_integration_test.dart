...
void main() {
  ...
  group('Token Granted Later Flow', () {
    test('updates token after initial registration', () async {
      ...
      await deviceService.updateFcmToken(fcmToken: 'new-fcm-token');
    ...
    test('handles token update before registration completes', () async {
      ...
      await deviceService.updateFcmToken(fcmToken: 'early-token');
  ...
  group('Token Rotation Flow', () {
    test('updates token when FCM rotates', () async {
      ...
      // Initial token
      await deviceService.updateFcmToken(fcmToken: 'token-v1');

      // FCM rotates token
      await deviceService.updateFcmToken(fcmToken: 'token-v2');
    ...
    test('handles null token (permission revoked)', () async {
      ...
      // Set initial token
      await deviceService.updateFcmToken(fcmToken: 'valid-token');
  ...
  group('Error Recovery', () {
    ...
    test('handles concurrent flush attempts safely', () async {
      ...
      deviceService.simulateOffline();
      await deviceService.updateFcmToken(fcmToken: 'token');
